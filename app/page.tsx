...
export default function 공사보고생성기() {
  ...
  useEffect(() => {
    const savedName = localStorage.getItem('현장명')
    const savedList = localStorage.getItem('현장목록')
    if (savedName) set현장명(savedName)
    if (savedList) set현장목록(JSON.parse(savedList))
  }, [])
  ...
  const handleToggle = (value: string, list: string[], setter: (v: string[]) => void) => {
      ...
      {결과 && (
        <Card>
          <CardContent className="whitespace-pre-wrap space-y-2">
            ...
            <Button
              onClick={() => {
                navigator.clipboard.writeText(결과)
                show('클립보드에 복사되었습니다.')
              }}
